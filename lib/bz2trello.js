...
(function() {
  ...
  function executeActions(data) {
    ...
    data.updatableCards.forEach(function(card) {
      actions.push(updateCard(card, data));
    });
  ...
  function createCard(bug, data) {
    return Promise.reject('Create card not implemented');
  }

  function updateCard(bug, data) {
    return Promise.reject('Updated card not implemented');
  }

  function discardCard(card, data) {
    console.log('noop: discarding card ' + card);
